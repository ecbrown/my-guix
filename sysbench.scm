    ...
    (arguments
     `(#:tests? #f ; until shebangs fixed
       ...
       (modify-phases %standard-phases
		      (delete 'bootstrap)
		      (add-after 'patch-source-shebangs 'libtoolize (lambda _ (invoke "libtoolize" "--copy" "--force")))
		      (add-after 'libtoolize 'aclocal (lambda _ (invoke "aclocal" "-I" "m4")))
		      (add-after 'aclocal 'autoreconf (lambda _ (invoke "autoreconf" "--install")))
		      (add-after 'autoreconf 'automake (lambda _ (invoke "automake" "-c" "--foreign" "--add-missing")))
		      (add-after 'automake 'autoconf (lambda _ (invoke "autoconf"))))))
    (native-inputs
     `(("autoconf" ,autoconf)
       ("automake" ,automake)
       ("libtool" ,libtool)
